...
if (isset($_GET['delete_id'])) {
    ...
    $sql_delete = "DELETE FROM recruiters WHERE id = ?";
    $stmt_delete = $conn->prepare($sql_delete);
    $stmt_delete->bind_param("i", $delete_id);
    $stmt_delete->execute();

    if ($stmt_delete->affected_rows > 0) {
        echo "Recruiter deleted successfully.";
    } else {
        echo "Error deleting recruiter.";
    }
}

// Fetch all recruiters
$sql = "SELECT * FROM recruiters ORDER BY id DESC";
...
<head>
    <meta charset="UTF-8">
    <title>Manage Recruiters - OnlineJobs.lk</title>
...
<center>
    <h2 class="jobs_txt">Manage Recruiters</h2>
        ...
        <tbody>
            <?php if ($result->num_rows > 0): ?>
                ...
                <tr>
                    ...
                    <td>
                        <!-- Update Button -->
                        <a href="update_recruiter.php?id=<?php echo $row['id']; ?>"><button>Update</button></a>
                        
                        <!-- Delete Button -->
                        <a href="manage_employers.php?delete_id=<?php echo $row['id']; ?>" onclick="return confirm('Are you sure you want to delete this recruiter?');">
            ...
            <?php else: ?>
                <tr>
                    <td colspan="7">No recruiters found.</td>
                </tr>
            <?php endif; ?>
        </tbody>
    </table>
